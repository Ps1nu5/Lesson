...
class Person(arcade.Sprite):
    def __init__(self):
        ...
        self.texture = self.idle_texture[0][RIGHT_FACING]
    ...
    def update_animation(self, delta_time: float = 1 / 30):
        ...
        if self.change_x == 0:
            ...
            if self.cur_texture % 2 == 0:
                ...
                self.texture = self.idle_texture[int(self.cur_texture)][self.person_face_direction]
        ...
        if not self.idle:
            ...
            if self.cur_texture >= 6:
                self.cur_texture = 0
            self.texture = self.run_texture[self.cur_texture][self.person_face_direction]
...
class NPC(arcade.Sprite):
    def __init__(self):
        ...
        for i in range(1, 9):
            ...
            self.idle_texture.append(texture)
        self.texture = self.idle_texture[0][LEFT_FACING]
    ...
    def update_animation(self, delta_time: float = 1 / 30):
        ...
        if self.cur_texture % 2 == 0:
            if self.cur_texture >= 8:
                self.cur_texture = 0
            self.texture = self.idle_texture[int(self.cur_texture)][LEFT_FACING]
...
class Game(arcade.Window):
    ...
    def setup(self):
        self.groud_list = arcade.SpriteList()
        ...
        for j in range(0, SCREEN_WIDTH+1, 16):
            ...
            ground.center_x = j
            ground.center_y = 5
            self.groud_list.append(ground)

        self.player = Person()
        self.player.center_x = 50
        self.player.bottom = self.groud_list[0].top

        self.physics_engine = arcade.PhysicsEnginePlatformer(self.player,
                                                             gravity_constant=GRAVITY,
                                                             walls=self.groud_list)

        self.npc = NPC()
        self.npc.center_x = 150
        self.npc.bottom = self.groud_list[0].top
